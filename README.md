...
The current app version is `0.1.5`.
...
- The top readouts show FFT peak frequency, peak axis, latest `Ax`, `Ay`, `Az`, and filtered `Tilt X` / `Tilt Y` pose angles with reserved numeric widths so changing values do not shift the top bar. The FFT peak marker stays hidden until live or simulated data is running.
...
Run `.github/workflows/release.yml` manually to publish a new release. Choose `patch`, `minor`, or `major`, or pass an exact SemVer version such as `0.1.5`. The workflow updates `package.json`, `package-lock.json`, `src-tauri/Cargo.toml`, and `src-tauri/tauri.conf.json`, commits the version bump, tags the release, and publishes the MSI plus updater metadata.
